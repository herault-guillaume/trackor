...
def get_price_for(session_prod,session_staging,session_id,buy_price_gold,buy_price_silver):
    ...
    delivery_ranges = [(0, 400, 10.0),(400.01, 800, 15.0), (800.01, 900, 21.0)] + [ (900.0 + (i * 100), 900.0 + ((i+1) * 100), 21.0 + (0.2 * (i+1))) for i,v in enumerate(range(900,15000,100))] + [(15000.01, float('inf'), 0.0)]
        ...
        try:
            ...
            for row in table.find_all('tr')[1:]:
                ...
                if "à" in quantity_str:
                    ...
                    price_ranges.append((1, float('inf'),Price.fromstring(price_str)))
                    if minimum is None:  # Save the first minimum
                        minimum = 1
            quantity = 1
