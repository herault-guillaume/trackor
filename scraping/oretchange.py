        ...
        try:
            ...
            if coin_name[name][:2] == 'or':
                coin = Item(name=coin_name[name],
                            ...
                            source=url,
                            buy_premium=((price.amount_float + 0.0) - (buy_price * poids_pieces[coin_name[name]])) * 100.0 / (buy_price * poids_pieces[coin_name[name]]),
                            delivery_fee=0.0, session_id=session_id, bullion_type='g')
            session.add(coin)
            session.commit()
